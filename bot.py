...
SEMANTIC_SEARCH_THRESHOLD = 0.66
SUGGESTION_THRESHOLD = 0.50
MAX_SUGGESTIONS_TO_SHOW = 2
CANDIDATES_TO_FETCH_FOR_SUGGESTIONS = 5

DYNAMIC_THRESHOLD_ENABLED = True
CONFIDENCE_GAP_FOR_DIRECT_ANSWER = 0.10
SIMILAR_SCORE_CLUSTER_THRESHOLD = 0.05
...
def build_semantic_embeddings():
    ...
    try:
        ...
        # Note: This means semantic search can find call codes too if their keywords match.
        # The original_idx for call_codes will need special handling or a different mapping
        # if we want to distinguish them from general_faqs in search results.
        # For now, let's just add them to the same embedding pool.
        # A more robust way would be separate embedding pools or tagging results.
        ...
        if isinstance(call_codes_dict, dict):
            for code_name, code_obj in call_codes_dict.items():
                if isinstance(code_obj, dict) and "keywords" in code_obj and "answer" in code_obj:
                    ...
                    # Create a "fake" general_faq_item index for call codes.
                    # This could be a string like "code_WRONG NUMBER" or a high number.
                    # For simplicity, let's make a string identifier.
        ...
        faq_original_indices = current_flat_faq_original_indices # Now contains integers (for general_faqs) and strings (for call_codes)
...
async def on_message(message: discord.Message):
    ...
    if user_query_lower_for_processing == "list codes" and not context_application_method == "list_item_resolved": 
        call_codes_section = faq_data.get("call_codes", {}) # Now expects {"CODE": {"keywords": [], "answer": "desc"}}
        cmd_log_extra = {'user_id': author_id, 'username': author_name, 'original_query_text': initial_user_message_content_for_forwarding}
        ...
        if not call_codes_section:
            ...
            for code_key in ordered_code_keys_for_context: 
                ...
                elif isinstance(code_obj, str): # Support old structure if accidentally mixed
    ...
    if match_define_code:
        ...
        call_codes_section = faq_data.get("call_codes", {}) # Expects {"CODE": {"keywords": [], "answer": "desc"}}
        ...
        if found_code_key:
        ...
        elif not context_was_applied_this_turn: 
            original_define_term_for_fuzzy = code_name_from_query_to_lookup 
            ...
            best_match_code, score = process.extractOne(original_define_term_for_fuzzy, list(call_codes_section.keys()), scorer=fuzz.token_set_ratio)
            ...
            if score > 80 and best_match_code:
                ...
                defined_code_embed = discord.Embed(title=f"Definition (for '{original_define_term_for_fuzzy}'): {best_match_code.upper()}", description=description_string_fuzzy, color=discord.Color.purple())
    ...
    call_codes_faq_section = faq_data.get("call_codes", {}) # For retrieving answers if a code keyword matches
        ...
        if top_indices:
            for i_log in range(min(len(top_indices), 3)): 
                ...
                if 0 <= idx_log < len(faq_original_indices) and 0 <= idx_log < len(faq_questions): 
                    original_faq_idx_ref = faq_original_indices[idx_log] # This can be int or string "code_..."
    ...
    if not top_indices: 
        ...
        if action == "direct_answer":
            ...
            if not (0 <= primary_faq_flat_idx < len(faq_questions) and 0 <= primary_faq_flat_idx < len(faq_original_indices)):
                ...
                original_faq_ref = faq_original_indices[primary_faq_flat_idx] # Can be int or string "code_..."
                ...
                if isinstance(original_faq_ref, int) and 0 <= original_faq_ref < len(faq_items_original_list): # It's a general_faq
                    ...
                    code_obj = call_codes_faq_section.get(code_name_from_ref) # Try to find by original casing if possible
                    if not code_obj: # Try upper if not found
                         code_obj = call_codes_faq_section.get(code_name_from_ref.upper())

                    if isinstance(code_obj, dict) and "answer" in code_obj:
                        faq_answer_part_text = code_obj["answer"]
                        # Embed for call code definition
                        ...
                        await message.channel.send(embed=embed)
                        # For logging, faq_answer_part_text is already set. bot_reply_parts will get this.
                ...
                if faq_answer_part_text and not (isinstance(original_faq_ref, str) and original_faq_ref.startswith("code_")): # Send if not already sent by embed
            ...
            for i in range(len(top_indices)): 
                ...
                if current_original_faq_ref not in shown_original_faq_refs:
                    ...
                    sugg_original_idx_for_button = current_original_faq_ref # Store the ref (int or str)
                    ...
                    if isinstance(current_original_faq_ref, int) and 0 <= current_original_faq_ref < len(faq_items_original_list):
                        ...
                        if isinstance(code_obj, dict) and "answer" in code_obj:
                            answer_text = f"Definition for **{code_name_from_ref.upper()}**: {code_obj['answer']}" # Prepend title for clarity
                        else:
                            logger.warning(f"Suggestion for code '{code_name_from_ref}' failed to find answer. Ref: {current_original_faq_ref}")
                            continue # Skip this suggestion if answer can't be found
                    else:
